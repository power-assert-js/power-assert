...
(function (root, factory) {
    ...
    function PowerAssertFormatter (dump) {
        this.dump = dump;
        this.initialVertivalBarLength = 1;
    }
    PowerAssertFormatter.prototype.addOneMoreRow = function () {
        this.rows.push(newRowFor(this.assertionLine));
    };
    PowerAssertFormatter.prototype.lastRow = function () {
        return this.rows[this.rows.length - 1];
    };
    PowerAssertFormatter.prototype.renderVerticalBarAt = function (columnIndex) {
        var lastRowIndex = this.rows.length - 1;
        for (var i = 0; i < lastRowIndex; i += 1) {
            this.rows[i].splice(columnIndex, 1, '|');
        }
    };
    PowerAssertFormatter.prototype.renderValueAt = function (columnIndex, dumpedValue) {
        var width = widthOf(dumpedValue);
        for (var i = 0; i < width; i += 1) {
            this.lastRow().splice(columnIndex + i, 1, dumpedValue.charAt(i));
        }
    };
    PowerAssertFormatter.prototype.constructRows = function (capturedEvents) {
        var that = this,
            prevCaptured;
    ...
    PowerAssertFormatter.prototype.initWithContext = function (context) {
    ...
    PowerAssertFormatter.prototype.formatLines = function () {
        var lines = [];
    ...
    PowerAssertFormatter.prototype.render = function (context) {
        var capturedEvents = context.events;
    ...
    return {
        ...
        format: function (context) {
            var formatter = new PowerAssertFormatter(this.dump);
            var lines = formatter.render(context);
            return lines;
        },
        PowerAssertFormatter: PowerAssertFormatter
    };
}));
