...
(function (root, factory) {
    ...
    var formatFuncallLines = function (cap, line, offset, dumper, identBuffers, funcallBuffers) {
        ...
        var point = cap.start + 1;
        //var point = Math.floor(((cap.start + cap.end) - Math.floor((cap.end - cap.start) * 0.8)) / 2);
        //var point = Math.floor((cap.start + cap.end) / 2);
        for (var i = 1; i < offset; i += 1) {
            //buffers[i].splice(cap.start, 1, '|');
            buffers[i].splice(point, 1, '|');
        }
        for (var i = 0; i < val.length; i += 1) {
            //buffers[offset].splice(cap.start + i, 1, val.charAt(i));
            buffers[offset].splice(point + i, 1, val.charAt(i));
        }
