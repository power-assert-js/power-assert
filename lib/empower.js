...
function instrumentExpression (expression, options) {
    ...
    instrumentor = new Instrumentor(retrieveLineFor(expression, options), options['powerAssertVariableName']);
    expression.arguments[0] = instrumentor.instrument(firstArgument);
}
...
function Instrumentor (line, powerAssertVariableName) {
    this.line = line;
    this.powerAssertVariableName = powerAssertVariableName;
}

Instrumentor.prototype.instrument = function (firstArgument) {
    var that = this,
        loc = that.locationOf(firstArgument),
        modified = that.captureRecursively(firstArgument);
    if (that.line) {
        return that.captureAssertion(modified, loc, that.line);
    } else {
        return that.captureAssertion(modified, loc);
    }
};
...
Instrumentor.prototype.captureAssertion = function (node, location, content) {
...
Instrumentor.prototype.captureIdent = function (target, location) {
    return this.capture('ident', target, location);
};

// enclose in _pa_.funcall
Instrumentor.prototype.captureFuncall = function (target, location) {
    return this.capture('funcall', target, location);
};

// enclose in _pa_.binary
Instrumentor.prototype.captureBinary = function (target, location) {
    return this.capture('binary', target, location);
};

Instrumentor.prototype.capture = function (kind, target, location) {
    return {
        type: "CallExpression",
        callee: {
            type: "MemberExpression",
