...
var capture = function (target, info) {
    return {
        "type": "CallExpression",
        "callee": {
...
var identToInfo = function (ident) {
    return {
        start: ident.loc.start.column,
        end: ident.loc.end.column
    };
};
...
var captureRecursively = function (node) {
    switch (node.type) {
    case 'Identifier':
        return capture(node, identToInfo(node));
    case 'MemberExpression': // ex: foo.bar.baz
        node.object = captureRecursively(node.object);
        return capture(node, identToInfo(node.property));
    case 'CallExpression':
        return capture(node, identToInfo(node.callee));
        // node.callee = captureRecursively(node.callee);
        // return node;
