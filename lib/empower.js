...
Instrumentor.prototype.propertyLocationOf = function (node) {
    var that = this,
        prop = node.property;
    if (typeof prop.loc !== 'undefined') {
        return locationOf(prop);
    } else if (typeof node.loc !== 'undefined') {
        var found = that.line.indexOf(prop.name, node.loc.start.column);
        if (found === -1) {
            throw new Error('Cannot detect property location: ' + prop.name + ' line: ' + that.line + ' col: ' + node.loc.start.column);
        }
        return locationOf({
            loc: {
                start: {
...
Instrumentor.prototype.captureRecursively = function captureRecursively(node) {
    ...
    case 'BinaryExpression':
        var endOfLeftColumn = node.left.loc.start.column + (node.left.range[1] - node.left.range[0]),
            operatorStartColumn = endOfLeftColumn + 1;
        if (that.line) {
            var found = that.line.indexOf(node.operator, endOfLeftColumn);
            if (found !== -1) {
